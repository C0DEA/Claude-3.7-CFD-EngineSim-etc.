...
class VariableEngineSimulator:
    ...
    def draw_engine(self):
        ...
        # Draw pistons and connecting rods first (they'll be behind the cylinders)
        self.draw_pistons_and_rods(crank_x, crank_y)
        
        # Draw each bank of cylinders with unified outlines
        self.draw_unified_cylinder_banks(crank_x, crank_y)
    ...
    def draw_pistons_and_rods(self, crank_x, crank_y):
        """Draw pistons and connecting rods separately"""
        # Define cylinder indices for left and right banks
        left_bank_indices = [0, 2, 4]  # Cylinders 1, 3, 5
        right_bank_indices = [1, 3, 5]  # Cylinders 2, 4, 6
        
        # Draw pistons for each bank
        for cyl_idx in range(CYLINDERS):
            # Determine which bank this cylinder belongs to
            if cyl_idx % 2 == 0:  # Cylinders 0, 2, 4 (1, 3, 5 in 1-based indexing)
                bank_idx = cyl_idx // 2
                bank_angle = -self.engine.v_angle / 2  # Left bank
                alpha = 255 - bank_idx * 80  # Transparency for depth
            else:  # Cylinders 1, 3, 5 (2, 4, 6 in 1-based indexing)
                bank_idx = cyl_idx // 2
                bank_angle = self.engine.v_angle / 2  # Right bank
                alpha = 255 - bank_idx * 80  # Transparency for depth
            
            # Get piston position
            piston_pos, _ = self.engine.piston_positions[cyl_idx]
            
            # Calculate pin angle and position
            ...
            # Bank direction vector
            bank_rad = math.radians(bank_angle)
            dir_x = math.sin(bank_rad)
            dir_y = -math.cos(bank_rad)
            
            # Cylinder dimensions and position - increased distance from crankshaft
            cylinder_height = 220
            ...
            piston_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            
            # Draw connecting rod
            rod_color = list(CONNECTING_ROD_COLOR) + [alpha]
            pygame.draw.line(piston_surface, rod_color, (pin_x, pin_y), (piston_x, piston_y), 8)
            
            # Draw piston
            piston_color = list(PISTON_COLOR) + [alpha]
            piston_rect = self.get_rotated_rect(
                piston_x, piston_y,
                60, 50,  # Piston dimensions
                math.degrees(bank_rad)
            )
            pygame.draw.polygon(piston_surface, piston_color, piston_rect)
            
            # Draw piston pin (black dot)
            pin_color = list(PIN_COLOR) + [alpha]
            pygame.draw.circle(piston_surface, pin_color, (int(piston_x), int(piston_y)), 5)
            
            # Blit to screen
            self.screen.blit(piston_surface, (0, 0))
        
    def draw_unified_cylinder_banks(self, crank_x, crank_y):
        """Draw cylinder banks with unified connected outlines"""
        # Calculate bank angles
        left_bank_angle = -self.engine.v_angle / 2
        right_bank_angle = self.engine.v_angle / 2
        
        # Create surfaces for each bank
        left_bank_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        right_bank_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        
        # Draw left bank (unified)
        self.draw_unified_bank(left_bank_surface, crank_x, crank_y, left_bank_angle, [0, 2, 4])
        
        # Draw right bank (unified)
        self.draw_unified_bank(right_bank_surface, crank_x, crank_y, right_bank_angle, [1, 3, 5])
        
        # Blit both banks to screen
        self.screen.blit(left_bank_surface, (0, 0))
        self.screen.blit(right_bank_surface, (0, 0))
    
    def draw_unified_bank(self, surface, crank_x, crank_y, bank_angle, cylinder_indices):
        """Draw a unified cylinder bank with connected outlines"""
        bank_rad = math.radians(bank_angle)
        dir_x = math.sin(bank_rad)
        dir_y = -math.cos(bank_rad)
        ...
        # Base position - increased distance from crankshaft
        cylinder_base_x = crank_x + self.engine.cylinder_offset * dir_x
        cylinder_base_y = crank_y + self.engine.cylinder_offset * dir_y
        ...
        # Calculate valve and spring positions
        valve_offset = 20
        valve_radius = 10
        spring_radius = 10
        
        # Calculate transparency (for the entire bank)
        alpha = 220
        
        # Calculate perpendicular direction to bank angle (for width of cylinders)
        perp_x = math.cos(bank_rad + math.pi/2)
        perp_y = math.sin(bank_rad + math.pi/2)
        
        # Calculate the unified outline
        outline_points = []
        
        # We'll create a unified shape by calculating key points along the bank
        # For the overall envelope of the cylinder bank
        
        # Calculate the offset for each cylinder
        # The cylinders in the image appear to overlap by about 1/3 of their width
        cylinder_spacing = cylinder_width * 2/3
        
        # Calculate the left and right edges of our bank
        left_offset = -cylinder_width/2
        right_offset = cylinder_width/2 + ((len(cylinder_indices) - 1) * cylinder_spacing)
        
        # Add bottom corners (at crankshaft end)
        outline_points.append((
            cylinder_base_x + left_offset * perp_x,
            cylinder_base_y + left_offset * perp_y
        ))
        outline_points.append((
            cylinder_base_x + right_offset * perp_x,
            cylinder_base_y + right_offset * perp_y
        ))
        
        # Add top corners (at valve end)
        outline_points.append((
            cylinder_top_x + right_offset * perp_x,
            cylinder_top_y + right_offset * perp_y
        ))
        outline_points.append((
            cylinder_top_x + left_offset * perp_x,
            cylinder_top_y + left_offset * perp_y
        ))
        
        # Draw the unified cylinder bank outline
        cylinder_color = list(CYLINDER_COLOR) + [alpha]
        pygame.draw.polygon(surface, cylinder_color, outline_points, 3)  # Just the outline, 3px wide
        
        # Now draw valves and springs for each cylinder in the bank
        for i, cyl_idx in enumerate(cylinder_indices):
            # Calculate the position along the bank for this cylinder
            offset = i * cylinder_spacing
            
            # Calculate center point for this cylinder along the bank
            cyl_center_x = cylinder_base_x + offset * perp_x
            cyl_center_y = cylinder_base_y + offset * perp_y
            
            # Calculate top of this cylinder (where valves are)
            cyl_top_x = cyl_center_x + cylinder_height * dir_x
            cyl_top_y = cyl_center_y + cylinder_height * dir_y
            
            # Calculate valve positions
            # Left valve
            valve1_x = cyl_top_x - valve_offset * perp_x
            valve1_y = cyl_top_y - valve_offset * perp_y
            valve_color = list(VALVE_COLOR) + [alpha]
            pygame.draw.circle(surface, valve_color, (int(valve1_x), int(valve1_y)), valve_radius)
            
            # Left spring
            spring1_x = valve1_x + 15 * dir_x
            spring1_y = valve1_y + 15 * dir_y
            spring_color = list(SPRING_COLOR) + [alpha]
            pygame.draw.circle(surface, spring_color, (int(spring1_x), int(spring1_y)), spring_radius)
            
            # Right valve
            valve2_x = cyl_top_x + valve_offset * perp_x
            valve2_y = cyl_top_y + valve_offset * perp_y
            pygame.draw.circle(surface, valve_color, (int(valve2_x), int(valve2_y)), valve_radius)
            
            # Right spring
            spring2_x = valve2_x + 15 * dir_x
            spring2_y = valve2_y + 15 * dir_y
            pygame.draw.circle(surface, spring_color, (int(spring2_x), int(spring2_y)), spring_radius)
