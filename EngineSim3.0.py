...
class StylizedEngineSimulator:
    ...
    def draw_engine(self):
        ...
        # Draw banks with cylinders and pistons
        self.draw_stylized_engine(crank_x, crank_y)
    ...
    def draw_stylized_engine(self, crank_x, crank_y):
        """Draw the stylized engine with cylinders and pistons"""
        # Calculate V bank angles
        left_bank_angle = -CYLINDER_ANGLE_V6 / 2
        right_bank_angle = CYLINDER_ANGLE_V6 / 2
        
        # Convert to radians
        left_bank_rad = math.radians(left_bank_angle)
        right_bank_rad = math.radians(right_bank_angle)
        
        # Base distances for layout
        cylinder_spacing = 90  # Spacing between cylinders in the same bank
        cylinder_bank_length = 250  # Length of the cylinder bank
        
        # Draw each bank
        left_cylinders = [0, 2, 4]  # Positions 0, 2, 4 for left bank
        right_cylinders = [1, 3, 5]  # Positions 1, 3, 5 for right bank
        
        # Draw left bank
        self.draw_cylinder_bank(crank_x, crank_y, left_bank_angle, left_cylinders, cylinder_spacing)
        
        # Draw right bank
        self.draw_cylinder_bank(crank_x, crank_y, right_bank_angle, right_cylinders, cylinder_spacing)
        
    def draw_cylinder_bank(self, crank_x, crank_y, bank_angle, cylinder_indices, spacing):
        """Draw a bank of cylinders with stylized appearance"""
        # Convert bank angle to radians
        bank_rad = math.radians(bank_angle)
        
        # Base position for first cylinder in bank
        base_x = crank_x
        base_y = crank_y
        
        # Direction vector along bank angle
        dir_x = math.sin(bank_rad)
        dir_y = -math.cos(bank_rad)
        
        # Draw cylinders in this bank
        for i, idx in enumerate(cylinder_indices):
            # Calculate cylinder position
            cylinder_pos = i - 1  # -1, 0, 1 for the three positions
            offset = cylinder_pos * spacing
            
            # Position of cylinder
            cyl_x = base_x + offset * math.cos(bank_rad)
            cyl_y = base_y + offset * math.sin(bank_rad)
            
            # Calculate transparency based on position (later cylinders more transparent)
            alpha = 255 - i * 60  # 255, 195, 135
            
            # Draw this cylinder
            self.draw_stylized_cylinder(cyl_x, cyl_y, bank_angle, idx, alpha)
            
    def draw_stylized_cylinder(self, x, y, angle, cylinder_idx, alpha):
        """Draw a single stylized cylinder with piston and rod"""
        # Convert angle to radians
        angle_rad = math.radians(angle)
        
        # Get piston position for this cylinder
        piston_pos, _ = self.engine.piston_positions[cylinder_idx]
        
        # Calculate crank pin position
        pin_angle = math.radians((self.engine.crank_angle + self.engine.crank_pin_angles[cylinder_idx]) % 360)
        pin_x = self.center_x + CRANK_RADIUS * math.cos(pin_angle)
        pin_y = self.center_y + 120 - CRANK_RADIUS * math.sin(pin_angle)
        ...
        cylinder_height = 250
        cylinder_top_x = x + cylinder_height * dir_x
        cylinder_top_y = y + cylinder_height * dir_y
        
        # Calculate piston position along cylinder axis
        piston_travel = cylinder_height * 0.6  # 60% of cylinder height
        piston_x = x + (piston_travel * piston_pos) * dir_x
        piston_y = y + (piston_travel * piston_pos) * dir_y
        ...
        # Draw cylinder body (rectangular)
        cylinder_width = 70
        cylinder_rect = self.get_rotated_rect(
            x + (cylinder_height/2) * dir_x, 
            y + (cylinder_height/2) * dir_y, 
            cylinder_width, cylinder_height, 
            math.degrees(angle_rad)
        )
        pygame.draw.polygon(cylinder_surface, cylinder_color, cylinder_rect)
        
        # Draw connecting rod
        pygame.draw.line(cylinder_surface, rod_color, (pin_x, pin_y), (piston_x, piston_y), 10)
        ...
        pygame.draw.polygon(piston_surface, (50, 50, 50, alpha), piston_rect, 2)
        
        # Draw pin holes (black dots) on piston
        pin_hole_radius = 6
        pygame.draw.circle(piston_surface, (0, 0, 0, alpha), (int(piston_x), int(piston_y)), pin_hole_radius)
        ...
        # Left valve (blue)
        valve1_x = cylinder_top_x - valve_offset * math.cos(angle_rad + math.pi/2)
        valve1_y = cylinder_top_y - valve_offset * math.sin(angle_rad + math.pi/2)
        ...
        # Right valve (blue)
        valve2_x = cylinder_top_x + valve_offset * math.cos(angle_rad + math.pi/2)
        valve2_y = cylinder_top_y + valve_offset * math.sin(angle_rad + math.pi/2)
        ...
        cylinder_num = FIRING_ORDER.index(cylinder_idx + 1) + 1
    ...
    def draw_throttle(self):
        ...
        self.screen.blit(throttle_label, (WIDTH - 100, HEIGHT // 2 - 180))
