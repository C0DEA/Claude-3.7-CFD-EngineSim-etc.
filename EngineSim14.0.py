...
class Engine:
    def __init__(self):
        ...
        # NEW: Track TDC crossings for sound generation
    ...
    def calculate_piston_positions(self):
        ...
        for i in range(CYLINDERS):
            ...
            # NEW: Apply BDC adjustment to prevent pistons from "touching" the crankshaft
...
class RealisticGTRSoundManager:
    def __init__(self, engine):
        ...
        # NEW: Create harmonic engine sounds (not using sound.wav)
        self.harmonic_sounds = []
        
        # Create harmonic sounds with variations for a more musical engine
        for i in range(6):  # Create different sounds for each cylinder
            self.harmonic_sounds.append(self.create_combustion_sound(variation=i))
    ...
    def create_combustion_sound(self, variation=0):
        """Create a harmonic engine sound instead of combustion sound"""
        sample_rate = 44100
        duration = 0.3  # longer for more harmonic sustain
        
        # Create a buffer for sound data
        buffer = np.zeros(int(sample_rate * duration))
        
        # Generate a harmonic sound with musical qualities
        t = np.linspace(0, duration, len(buffer), False)
        
        # Base frequency - varies slightly per cylinder for harmony
        base_freq = 80 + variation * 5
        
        # Create a harmonic series with proper musical ratios
        # These create a more pleasing musical structure
        harmonics = [
            (1.0, 0.8),    # fundamental (base freq)
            (2.0, 0.5),    # octave
            (3.0, 0.3),    # perfect fifth + octave
            (4.0, 0.2),    # second octave
            (5.0, 0.1),    # major third + second octave
            (6.0, 0.05)    # perfect fifth + second octave
        ]
        
        # Build the harmonic tone
        for freq_ratio, amplitude in harmonics:
            freq = base_freq * freq_ratio
            buffer += amplitude * np.sin(2 * np.pi * freq * t)
        
        # Apply envelope for a natural sound
        ...
        vibrato_freq = 12  # Hz
        vibrato_amount = 0.5
        vibrato = vibrato_amount * np.sin(2 * np.pi * vibrato_freq * t)
        
        # Apply the vibrato
        for i, time in enumerate(t):
            buffer[i] += vibrato[i] * np.sin(2 * np.pi * (base_freq * 2) * time) * envelope[i] * 0.2
    ...
    def update(self):
        ...
        if self.idle_playing:
            ...
            # Skip additional sounds in slow mode for clarity
            if not self.engine.slow_mode:
        ...
        # Calculate time between explosion sounds based on RPM
        current_time = time.time()
        
        # Play harmonious sounds at TDC for each cylinder
        for cyl_idx in range(CYLINDERS):
            # See if this cylinder just hit TDC
            if self.engine.is_cylinder_at_tdc(cyl_idx):
                # Don't fire too frequently
                if current_time - self.last_fire_time > 0.05:  # Min time between consecutive sounds
                    # Use the sound for this cylinder
                    harmonic_sound = self.harmonic_sounds[cyl_idx]
                    ...
                    # Calculate volume based on RPM and throttle
                    volume = min(1.0, 0.3 + 0.7 * self.engine.throttle)
                    
                    # Play the sound
                    channel.play(harmonic_sound)
...
class VariableEngineSimulator:
    ...
    def draw_bank(self, crank_x, crank_y, bank_angle, cylinder_indices):
        ...
        # Draw cylinders in this bank from back to front - keep overlapping cylinders
        for i, cyl_idx in reversed(list(enumerate(cylinder_indices))):
            # Calculate transparency for depth perception
            alpha = 255 - i * 80
            
            # Get piston position - normalize to ensure same ATH and bottom
            ...
            cylinder_height = 200
            cylinder_width = 70
            cylinder_spacing = 0  # Offset between cylinders in the same bank
            
            # Calculate position along the bank - keep original overlapping layout
            ...
            # Draw cylinder as a hollow rectangle
            ...
            # Draw inner cylinder with gray fill
            cylinder_color = list(CYLINDER_COLOR) + [alpha]
            pygame.draw.polygon(cylinder_surface, cylinder_color, cylinder_points, 0)  # Filled polygon
            
            # Draw cylinder walls with pink outline
            wall_color = list(CYLINDER_WALL_COLOR) + [alpha]
            pygame.draw.polygon(cylinder_surface, wall_color, cylinder_points, 3)  # Thicker border
            
            # Draw connecting rod
            rod_color = list(CONNECTING_ROD_COLOR) + [alpha]
            
            # Calculate piston position within the cylinder
            piston_travel = cylinder_height * 0.7  # Reduced travel to keep pistons from touching crankshaft
