...
# Engine dimensions
PISTON_WIDTH = 55  # Slightly narrower to fit inside cylinder
PISTON_HEIGHT = 60
CYLINDER_WIDTH = 60  # Closer to piston width for tighter fit
CYLINDER_HEIGHT = 220
CYLINDER_WALL_THICKNESS = 3
ROD_LENGTH = 120  # Fixed rod length
CRANK_RADIUS = 40  # This determines stroke length
CYLINDER_OFFSET = 50  # Distance from crank center to cylinder base

# Consistent stroke length for all cylinders
STROKE_LENGTH = CRANK_RADIUS * 2  # Stroke equals twice the crank radius
...
class Engine:
    ...
    def calculate_piston_positions(self):
        """Calculate piston positions with uniform stroke for all cylinders"""
        for i in range(CYLINDERS):
            ...
            # Direct trigonometric calculation for consistent stroke
            # Up-down motion is purely cosine based for equal travel (stroke)
            # This ensures all pistons travel the same distance regardless of bank angle
            # 0 = TDC (top), 1 = BDC (bottom)
            piston_pos = (1 - math.cos(effective_rad)) / 2
...
class PrecisionEngineSimulator:
    def __init__(self):
        pygame.init()
        self.screen = pygame.display.set_mode((WIDTH, HEIGHT))
        pygame.display.set_caption("Precision Engine Simulator - Nissan GTR VR38DETT")
    ...
    def draw_bank(self, crank_x, crank_y, bank_angle, cylinder_indices):
        ...
        for i, cyl_idx in reversed(list(enumerate(cylinder_indices))):
            ...
            # Use a consistent stroke/travel distance for all pistons
            ...
            piston_width = PISTON_WIDTH  # Slightly smaller than cylinder for tight fit
            ...
            # Calculate the inner and outer boundaries of cylinder (for "hugging" effect)
            # We'll draw two polygons - outer in pink, inner in transparent/background color
            outer_half_width = cylinder_width / 2
            inner_half_width = piston_width / 2 + 2  # Just slightly larger than piston
            
            # Calculate the four corners for outer cylinder boundary
            outer_cylinder_points = [
                (cyl_center_x - outer_half_width * perp_x, cyl_center_y - outer_half_width * perp_y),
                (cyl_center_x + outer_half_width * perp_x, cyl_center_y + outer_half_width * perp_y),
                (cyl_top_x + outer_half_width * perp_x, cyl_top_y + outer_half_width * perp_y),
                (cyl_top_x - outer_half_width * perp_x, cyl_top_y - outer_half_width * perp_y)
            ]
            
            # Calculate inner corners for the hollow part
            ...
            # Draw outer cylinder shape (pink)
            cylinder_color = list(CYLINDER_COLOR) + [alpha]
            pygame.draw.polygon(cylinder_surface, cylinder_color, outer_cylinder_points, 0)
            
            # Draw inner cylinder (hole) with background color
            inner_color = list(BACKGROUND_COLOR) + [alpha]
            pygame.draw.polygon(cylinder_surface, inner_color, inner_cylinder_points, 0)
            
            # Draw valve and spring details consistently positioned
            valve_spacing = 15
            valve_radius = 10
            spring_radius = 10
            
            # Position valves symmetrically at the top of the cylinder
            # First valve-spring pair
            valve1_x = cyl_top_x - valve_spacing * perp_x
            valve1_y = cyl_top_y - valve_spacing * perp_y
            ...
            spring1_x = valve1_x + 12 * dir_x
            spring1_y = valve1_y + 12 * dir_y
            ...
            # Second valve-spring pair
            valve2_x = cyl_top_x + valve_spacing * perp_x
            valve2_y = cyl_top_y + valve_spacing * perp_y
            pygame.draw.circle(cylinder_surface, valve_color, (int(valve2_x), int(valve2_y)), valve_radius)
            
            # Spring behind the valve
            spring2_x = valve2_x + 12 * dir_x
            spring2_y = valve2_y + 12 * dir_y
...
if __name__ == "__main__":
    simulator = PrecisionEngineSimulator()
    simulator.run()