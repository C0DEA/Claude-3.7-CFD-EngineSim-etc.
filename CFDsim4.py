...
class CFDVisualizationApp(QMainWindow):
    ...
    def display_pressure(self, auto_scale=False):
        ...
        # Define absolute pressure scale based on dynamic pressure
        # Dynamic pressure = 0.5 * density * velocity^2
        # This ensures the color scale changes with velocity squared
        dynamic_pressure = 0.5 * self.cfd_solver.density * self.cfd_solver.airspeed**2
        
        # Set scale based on dynamic pressure - using a much more aggressive scaling
        # Increasing sensitivity factor from 10.0 to 2.0 to show more contrast
        # This will make high-speed simulations show more extreme colors
        pressure_scale = 2.0 * dynamic_pressure
        
        # Tighter pressure range to emphasize high and low pressure areas
        pressure_min = -0.7 * pressure_scale  # Negative pressure (suction)
        pressure_max = 1.0 * pressure_scale   # Positive pressure (stagnation)
        
        # Normalize the pressures to match the new scale
        # This ensures the colors will properly distribute across the model
        normalized_pressures = pressures * (pressure_scale / dynamic_pressure)
        
        # Set the color range - this will determine how colors map to pressure values
        lut.SetTableRange(pressure_min, pressure_max)
        lut.SetHueRange(0.667, 0.0)  # Blue to red
        lut.Build()
        
        # Map pressures to colors using the normalized pressures for better contrast
    ...
    def update_pressure_scale(self, min_p, max_p, lut):
        ...
        # Set x axis ticks to pressure values with scientific notation for large values
        ax.set_xticks([0, 127, 255])
        
        # Use scientific notation for large pressure values
        if abs(min_p) > 1000 or abs(max_p) > 1000:
            ax.set_xticklabels([f"{min_p:.1e}", f"{(min_p + max_p) / 2:.1e}", f"{max_p:.1e}"])
        else:
            ax.set_xticklabels([f"{min_p:.1f}", f"{(min_p + max_p) / 2:.1f}", f"{max_p:.1f}"])
