...
class Engine:
    def __init__(self):
        ...
        # Position cylinder further away from crankshaft
        self.cylinder = Cylinder(self.center_x, self.center_y - self.rod_length - self.crank_radius - self.cylinder_offset, 
    ...
    def update(self, dt):
        if self.running:
            ...
            # Update valves based on crankshaft position (simplified 4-stroke cycle)
            # Intake valve opens during intake stroke (0-180 degrees)
            # Exhaust valve opens during exhaust stroke (540-720 degrees)
            four_stroke_angle = (self.angle * 360 / (2 * math.pi)) % 720
            
            # Intake valve timing
            if 0 <= four_stroke_angle < 220:
                intake_lift_percentage = math.sin(four_stroke_angle * math.pi / 220)
                self.intake_valve.update(lift_percentage=intake_lift_percentage)
            else:
                self.intake_valve.update(lift_percentage=0)
            
            # Exhaust valve timing
            if 480 <= four_stroke_angle < 720:
                exhaust_lift_percentage = math.sin((four_stroke_angle - 480) * math.pi / 240)
                self.exhaust_valve.update(lift_percentage=exhaust_lift_percentage)
            else:
...
class Cylinder:
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen, PINK, 
                         (self.x, self.y, self.width, self.wall_thickness * 2))
...
class Valve:
    ...
    def draw(self, screen):
        ...
        pygame.draw.rect(screen, LIGHT_GRAY, 
                         (self.x - 4, self.y - self.stem_length + self.current_lift, 
                          8, self.stem_length))
        
        # Draw valve head
        valve_head_y = self.y + self.current_lift
        pygame.draw.ellipse(screen, self.color, 
                           (self.x - self.radius, valve_head_y - self.radius // 2, 
