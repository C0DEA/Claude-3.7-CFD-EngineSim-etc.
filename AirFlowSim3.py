...
class AeroStreamlinesViewer(QtWidgets.QMainWindow):
    ...
    def setup_velocity_field(self):
        ...
        self.vector_field.SetDimensions(30, 30, 30)  # Lower resolution for better performance
        self.vector_field.SetExtent(0, 29, 0, 29, 0, 29)
        self.vector_field.SetOrigin(bounds_with_margin[0], bounds_with_margin[2], bounds_with_margin[4])
        self.vector_field.SetSpacing(
            (bounds_with_margin[1] - bounds_with_margin[0]) / 29,
            (bounds_with_margin[3] - bounds_with_margin[2]) / 29,
            (bounds_with_margin[5] - bounds_with_margin[4]) / 29
        )
    ...
    def update_vector_field(self):
        ...
        for i in range(num_points):
            ...
            # Calculate simplified flow deflection around model using locator
            closest_point = [0, 0, 0]
    ...
    def create_streamlines(self):
        ...
        # Decide on number of seed points (fewer for better performance)
        base_points = 4  # Reduced from 6
        num_y_points = max(2, int(base_points * density_factor))
        num_z_points = max(2, int(base_points * density_factor))
