...
class SimplifiedCFDSolver:
    ...
    def calculate_pressure(self):
        ...
        # Amplify the effect to create more visual contrast
        cp = 1.5 * dots - 0.2  # Amplified pressure coefficient for better visualization
...
class CFDVisualizationApp(QMainWindow):
    ...
    def add_flow_direction_indicator(self):
        ...
        # This method is intentionally empty now as we're removing the flow direction arrow
        pass
        
    def update_flow_direction_arrow(self):
        """Update the flow direction arrow based on current pitch and yaw angles"""
        # This method is now empty as we've removed the flow direction arrow
        pass
    ...
    def update_yaw(self):
        """Update the yaw angle in the CFD solver (for backward compatibility)"""
    ...
    def run_cfd_analysis(self):
        ...
        # Streamline generation is typically the slowest part, so allocate progress accordingly
        # Pressure calculation: 0-40%
        # Streamline generation: 40-95%
        # Finishing: 95-100%
        ...
        # This is the most computation-intensive step, so we'll update the bar less frequently
        self.progress_bar.setValue(41)
        QApplication.processEvents()
        # Start streamline generation in smaller chunks to keep UI responsive
        ...
        # Create streamline actors now that calculation is done
    ...
    def display_pressure(self, auto_scale=False):
        ...
        # Create a color lookup table with fixed scale
        lut = vtk.vtkLookupTable()
        
        # Use a fixed reference pressure scale for the legend display
        # but DO NOT scale the actual pressures when coloring the model
        reference_speed = 25.0  # m/s (reduced reference speed for better color distribution)
        reference_dynamic_pressure = 0.5 * self.cfd_solver.density * reference_speed**2
        
        # Fixed pressure scale for the color map legend
        pressure_scale = 2.0 * reference_dynamic_pressure
        pressure_min = -0.7 * pressure_scale  # Negative pressure (suction)
        pressure_max = 1.0 * pressure_scale   # Positive pressure (stagnation)
        
        # Set the color table to use the fixed scale for the legend
        lut.SetTableRange(pressure_min, pressure_max)
        lut.SetHueRange(0.667, 0.0)  # Blue to red
        lut.Build()
        
        # Real pressure values still vary by current airspeed
        # This keeps the scale fixed but the actual colors will still vary with speed
        
        # Map pressures to colors based on the fixed scale
        # DO NOT scale the pressures - this will allow higher speeds to naturally 
        # show more extreme colors (more reds and blues)
        ...
        # Update the pressure scale visualization with absolute scale
    ...
    def update_pressure_scale(self, min_p, max_p, lut):
        ...
        # Fixed pressure values in the legend - now the scale doesn't change with airspeed
        ax.set_xticks([0, 127, 255])
        
        # Format the values nicely
        formatter = lambda x: f"{x:.1e}" if abs(x) > 1000 else f"{x:.1f}"
        ax.set_xticklabels([formatter(min_p), formatter((min_p + max_p) / 2), formatter(max_p)])
